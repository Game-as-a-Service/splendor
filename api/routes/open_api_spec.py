...
class OpenApiSpec:
    location = os.path.realpath(os.path.join(os.path.realpath(os.path.dirname(__file__)), '..', 'open-api-spec'))
    ...
    def resolve_content(cls, path: str or None) -> Response:
        ...
        if abs_path.endswith(('.yaml', '.yml', '.json')):
            ...
            if abs_path.endswith('.json'):
                return Response(json.dumps(resolved, ensure_ascii=False), 200, {'Content-Type': 'application/json'})
