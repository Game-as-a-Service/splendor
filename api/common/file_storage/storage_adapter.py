...
class S3FileStorage(IFileStorage):
    ...
    def delete(self, path: str, recursive: bool = False):
        ...
        while next_marker:
            ...
            while delete_objects:
                self._s3_client.delete_objects(Bucket=self._bucket, Delete={'Objects': delete_objects})
                s += interval
                delete_objects = objects[s:s + interval]
    ...
    def stat(self, path: str) -> FileStat:
        ...
        res = self._s3_client.list_objects(Bucket=self._bucket, Prefix=prefix, Delimiter='/', MaxKeys=1)
        ...
        if not contains and common_prefixes[0]['Prefix'] == path.strip('/') + '/':
            ...
            contains = res.get('Contents')
        if not contains:
            file_type = TYPE_DIRECTORY if common_prefixes[0]['Prefix'].endswith('/') else TYPE_FILE
            return FileStat(
                type=file_type,
        ...
        if prefix != contains[0]['Key'] and contains[0]['Key'].startswith(prefix.strip('/') + '/'):
            return FileStat(
                type=TYPE_DIRECTORY,
                creation_time=datetime.fromtimestamp(0),
                modification_time=datetime.fromtimestamp(0),
    ...
    def path_join(path: str, *paths: str) -> str:
        ...
        return ('/'.join([path.strip('/'), *[r.strip('/') for r in paths if r.strip('/')]]) + tail_slash).lstrip('/')
