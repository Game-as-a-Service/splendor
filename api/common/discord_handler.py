...
class DiscordHandler(logging.Handler):
    ...
    def attempt_to_report_failure(self, resp: Response, orignal: DiscordWebhook):
        ...
        print(  # noqa : T201
            ...
            "{orignal.content}, embeds: {orignal.embeds}", file=sys.stderr)
    ...
    def emit(self, record: logging.LogRecord):  # noqa: C901
        ...
        try:
            ...
            embed = DiscordEmbed(title=f"{emoji}  {self._service_name} ({self._environment})",
            ...
            embed.add_embed_field(name="DATETIME", value=f"```{fields[0]}```", inline=True)
            embed.add_embed_field(name="HOSTNAME", value=f"```{socket.gethostname()}```", inline=False)
            embed.add_embed_field(name="ERROR MESSAGE", value=f"```{fields[5]}```", inline=False)
