...
class SystemMailer:
    ...
    def send_billing_info_mail(self, payload: dict) -> None:
        url = self._config.GROWIN_MAIL_URL.rstrip("/") + "/send/nike-billing-info-mail"
    ...
    def send_trial_plan_mail(self, email: str) -> None:
        url = self._config.GROWIN_MAIL_URL.rstrip("/") + "/send/nike-free-trial-mail"

        payload = {
            "recipient": email,
            "env": self._config.ENV_SET
        }
        res = requests.post(url=url, json=payload)
