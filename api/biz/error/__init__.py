...
def guard_valid_param(
    ...
    guard_then_raise(condition, err_msg, InvalidInvocation, fill_errors, logger, extra_log_msg)
...
def guard_valid_data(
    ...
    guard_then_raise(condition, err_msg, DataValidationError, fill_errors, logger, extra_log_msg)
...
def guard_business_rule(
    ...
    guard_then_raise(condition, err_msg, BusinessError, fill_errors, logger, extra_log_msg)
...
def guard_then_raise(
    ...
    if not condition:
        if isinstance(logger, Logger):
            logger.warning(err_msg if not extra_log_msg else f'{err_msg}, {extra_log_msg}')
